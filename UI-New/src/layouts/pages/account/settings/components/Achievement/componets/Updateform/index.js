...
function Updateform({ Selecteddata, sethide, fetchData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.put(
        `/_v1/user/achievement/update/${Selecteddata.id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response) {
        fetchData();
        sethide(true);
        addToast("Document Update successfully", { appearance: "success" });
      }
    } catch (error) {
      console.error("Error submitting form:", error);
  ...
  return (
    <div>
      <SoftBox p={3}>
        <Typography variant="h5">Update Achievement Details</Typography>
      </SoftBox>
      <SoftBox component="form" pb={3} px={3}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={6}>
            <label className="text-xs font-bold p-1">
            achievementDescription<span style={{ color: "red" }}>*</span>
            </label>
            <SoftInput
